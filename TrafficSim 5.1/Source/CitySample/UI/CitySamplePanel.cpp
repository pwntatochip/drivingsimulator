...
UCitySamplePanel* UCitySamplePanel::AddChildPanel(UCitySamplePanel* ChildPanel, UPanelWidget* ContainerWidget, bool bSkipAnimation)
{
	...
	{
		/*
		 * TODO: We are forcing an animation skip here and not setting pending state data if the previous parent == the new parent.
		 * Ideally, we'd be able to execute the same behavior regardless however  UCitySamplePanel::NativeDestruct() will cause a child's parent panel to remove its child.
		 * In the case where you try to re-add a child panel to the same parent this will result in a remove instead of a refresh.
		 * Destruction logic should be fixed to account for this, but for now instantly removing and proceeding with the function fixes the issue.
		 * If for whatever reason you'd like a panel refresh to trigger an outro animation, please formally remove the child panel before re-adding it.
		 */
		if (PreviousParent == this)
		{
			PreviousParent->RemoveChildPanelChecked_Internal(ChildPanel, true);
		}
		else
		{
