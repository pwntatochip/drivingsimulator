...
void UCitySampleMapWidget::NativeOnInitialized()
{
	...
	const bool bMapImageIsATexture = MapImage->Brush.GetResourceObject()->IsA(UTexture2D::StaticClass());

	if (bMapImageIsATexture)
	{
		const FIntPoint MapBaseSize = MapImage->Brush.GetImageSize().IntPoint() / 2;
